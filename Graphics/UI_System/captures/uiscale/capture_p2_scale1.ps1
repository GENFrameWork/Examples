...
# Fase 2 scale=1.0 (input map): launch UI_System, capture Resumen/CPU/Memoria/Disco via PrintWindow + nav clicks.
...
function Capture-Window([IntPtr]$hwnd, [string]$path) {
  $r = New-Object Win32UiscaleP2+RECT
  [Win32UiscaleP2]::GetWindowRect($hwnd, [ref]$r) | Out-Null
  $w = $r.Right - $r.Left
  $h = $r.Bottom - $r.Top
  ...
  for ($attempt = 1; $attempt -le 4; $attempt++) {
    [Win32UiscaleP2]::SetForegroundWindow($hwnd) | Out-Null
    Start-Sleep -Milliseconds 250
    $bmp = New-Object System.Drawing.Bitmap $w, $h
    $g = [System.Drawing.Graphics]::FromImage($bmp)
    if ($attempt -le 2) {
      $hdc = $g.GetHdc()
      [Win32UiscaleP2]::PrintWindow($hwnd, $hdc, 2) | Out-Null
      $g.ReleaseHdc($hdc)
    } else {
...
function Click-Client([IntPtr]$hwnd, [int]$cx, [int]$cy) {
  $pt = New-Object Win32UiscaleP2+POINT
  $pt.X = $cx; $pt.Y = $cy
  [Win32UiscaleP2]::ClientToScreen($hwnd, [ref]$pt) | Out-Null
  [Win32UiscaleP2]::SetCursorPos($pt.X, $pt.Y) | Out-Null
  Start-Sleep -Milliseconds 80
  [Win32UiscaleP2]::mouse_event([Win32UiscaleP2]::MOUSEEVENTF_LEFTDOWN, 0, 0, 0, [UIntPtr]::Zero)
  Start-Sleep -Milliseconds 40
  [Win32UiscaleP2]::mouse_event([Win32UiscaleP2]::MOUSEEVENTF_LEFTUP, 0, 0, 0, [UIntPtr]::Zero)
...
[Win32UiscaleP2]::ShowWindow($hwnd, 9) | Out-Null
[Win32UiscaleP2]::SetForegroundWindow($hwnd) | Out-Null
Start-Sleep -Milliseconds 600

$wr = New-Object Win32UiscaleP2+RECT
$cr = New-Object Win32UiscaleP2+RECT
[Win32UiscaleP2]::GetWindowRect($hwnd, [ref]$wr) | Out-Null
[Win32UiscaleP2]::GetClientRect($hwnd, [ref]$cr) | Out-Null
$ww = $wr.Right - $wr.Left; $wh = $wr.Bottom - $wr.Top
$cw = $cr.Right - $cr.Left; $ch = $cr.Bottom - $cr.Top
@"
# Fase 2 — medidas de ventana (runtime)
...
"@ | Set-Content -Path (Join-Path $outDir "P2_scale1_window_metrics.md") -Encoding UTF8
Write-Host "window=${ww}x${wh} client=${cw}x${ch}"

# Default section = Resumen
Start-Sleep -Seconds 1
Capture-Window $hwnd (Join-Path $outDir "P2_scale1_resumen.png")
...
foreach ($item in $map) {
  ...
  Capture-Window $hwnd (Join-Path $outDir ("P2_scale1_{0}.png" -f $item.Name))
}
...
Start-Sleep -Seconds 1
Capture-Window $hwnd (Join-Path $outDir "P2_scale1_disco_page2.png")
Click-Client $hwnd 682 786
Start-Sleep -Seconds 1
Capture-Window $hwnd (Join-Path $outDir "P2_scale1_disco_page1.png")

Write-Host "P2 scale1 captures done"
Get-ChildItem $outDir -Filter "P2_scale1_*.png" | Select-Object Name, Length
